...
99AI Plugin Edition 是一套**完整未编译**的开源 AI 服务平台。用户在聊天端对话；管理员在后台配置模型、应用和站点数据。仓库同时包含用户端、管理端和服务端，适合私有化部署和二次开发。

部署需要 MySQL、Redis，以及至少一个 **OpenAI 兼容**模型服务。整合部署后默认监听 `9520`：用户端在 `/`，管理端在 `/admin`。

## 适合谁

- **想自己搭一套 AI 站点的人。** Docker 或一键打包先让对话跑起来，运营配置可以后补。
- **需要的不只是一个聊天框的团队。** 用户、额度、套餐、登录方式和站点外观都在管理端。
- **要改源码的开发者。** 仓库里是 NestJS + Vue 3，插件、Agent、MCP 都有明确入口。

如果你只要一个最小聊天窗，它可能偏重。如果你希望对话、插件和运营后台在同一套产品里，它就是为此准备的。

## 核心能力

- **多模型对话** — 上下文、流式回复；模型 Key 在管理端填写，不写进源码。
...
生产加固、HTTPS、备份和上线验收见 [部署与首次配置](docs/DEPLOYMENT_AND_CONFIGURATION.md)。
...
编辑 `.env`，至少设置以下项。`INITIAL_ADMIN_PASSWORD` 示例值为空，占位符会被拒绝。
...
- 用户端：`http://<主机>:9520/`
- 管理端：`http://<主机>:9520/admin`
- 健康检查：`GET /api/health`

### 方式二：一键整合打包（`./build.sh`）

适合已经有 MySQL / Redis，希望先在本机构建再拷到服务器的场景。仓库根目录的脚本会安装锁定依赖、构建三端，并输出到独立目录：
...
构建环境需要 Node.js 20–24 和 pnpm 10.33.2。`99AIPluginQuickDeploy/` 不会进入 Git 或 Docker 构建上下文；再次执行脚本时，只有三端全部构建成功才会更新该目录。整合包目录说明见 [docs/QUICK_DEPLOY.md](docs/QUICK_DEPLOY.md)。
...
模型服务 Key 由部署者在管理端填写。正式对外前，请用真实配置验证支付、邮件、短信和微信回调。
