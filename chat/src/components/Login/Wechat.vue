...
async function getSeneStr() {
  ...
  if (res.success) {
    ...
    confirmationCode.value = res.data.confirmationCode
    getQrCodeUrl()
  }
}

async function loginBySnece() {
  if (!sceneStr.value || !pollToken.value) return
...
async function getQrCodeUrl() {
  ...
  if (res.success) {
    ...
    timer.value = setInterval(() => {
      if (Date.now() - timerStartTime.value > 120000) {
        clearInterval(timer.value)
        return
      }
      loginBySnece()
...
onMounted(() => {
  ...
  countdownTimer.value = setInterval(handleTimeDown, 120000)
  // getSeneStr();
})
  ...
  <div class="w-full h-full flex flex-col justify-between" :class="isMobile ? 'px-5 ' : 'px-10 '">
    <div class="flex flex-col items-center flex-1">
      ...
      <p class="text-sm text-gray-600 dark:text-gray-400 mb-2">
        {{ t('login.scanQrCodeToLogin') }}
      </p>
      <p class="max-w-[280px] text-center text-xs text-amber-600 dark:text-amber-400 mb-2">
        {{ t('login.confirmQrInWechat') }}
      </p>
      <p class="text-sm font-semibold tracking-[0.25em] text-gray-800 dark:text-gray-200 mb-2">
        {{ confirmationCode }}
      </p>

      <div v-if="Number(globalConfig.isAutoOpenAgreement) === 1" class="flex items-center mt-2">
        <input
        ...
        <p class="ml-2 text-sm text-gray-600 dark:text-gray-400">
          {{ t('login.scanMeansAgree') }}
          <a
            href="#"
            class="font-medium text-primary-600 hover:text-primary-500 dark:text-primary-400 dark:hover:text-primary-300"
            @click="handleClick"
            >{{ t('login.userAgreementAndPrivacy') }}</a
          >
        </p>
      </div>
    </div>
