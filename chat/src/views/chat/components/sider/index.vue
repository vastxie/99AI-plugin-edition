...
const siteName = authStore.globalConfig?.siteName || '99AI Plugin Edition'
...
const pluginFirst = computed(() => Number(authStore.globalConfig.pluginFirst) === 1)
const isHidePlugin = computed(() => Number(authStore.globalConfig.isHidePlugin) === 1)

// const addLoading = ref(false);
const avatar = computed(
  () => authStore.userInfo.avatar || authStore.globalConfig.userDefaultAvatar || ''
)

const collapsed = computed(() => appStore.siderCollapsed)

const activeSideOption = ref(!pluginFirst.value || isHidePlugin.value ? 'chat' : 'plugin')
...
const displayInfo = computed(() => {
  ...
  if (remainingPoints > 99999) {
    remainingPoints = t('member.unlimited')
    buttonText = t('member.times')
  }
...
onMounted(() => {
  ...
  // 应用列表和模型列表已在 App.vue 中初始化，这里不需要重复查询
})
    ...
    >
      ...
      >
        <main class="flex flex-col h-full flex-1">
          ...
          <div v-if="!isHidePlugin" class="flex w-full px-4 my-2">
            <div class="tab-group tab-group-default">
              <button
                v-if="pluginFirst"
                @click="activeSideOption = 'plugin'"
                class="tab tab-md"
                :class="{ 'tab-active': activeSideOption === 'plugin' }"
                :aria-label="`切换到${t('common.pluginTab')}`"
              >
                {{ t('common.pluginTab') }}
              </button>
              <button
                ...
                class="tab tab-md"
                :class="{ 'tab-active': activeSideOption === 'chat' }"
                :aria-label="`切换到${t('common.conversationTab')}`"
              >
                {{ t('common.conversationTab') }}
              </button>
              <button
                v-if="!pluginFirst"
                @click="activeSideOption = 'plugin'"
          ...
          >
            ...
            >
              ...
              <div v-if="!isMobile" class="tooltip tooltip-top">{{ t('common.settings') }}</div>
            </div>
            ...
            >
              <button
                type="button"
                class="btn btn-pill mb-2 w-full"
                :aria-label="`${t('common.login')} / ${t('common.register')}`"
              >
