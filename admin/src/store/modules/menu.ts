...
const useMenuStore = defineStore(
  ...
  () => {
    ...
    function getDeepestPath(routeMenu: Menu.recordRaw, rootPath = '') {
      let retnPath = '';
      if (routeMenu.children) {
        const visibleChild = routeMenu.children.find((item) => item.meta?.menu !== false);
        if (visibleChild) {
          retnPath = getDeepestPath(visibleChild, resolveRoutePath(rootPath, routeMenu.path));
        } else {
          retnPath = getDeepestPath(
            routeMenu.children[0],
            resolveRoutePath(rootPath, routeMenu.path),
          );
        }
      } else {
        retnPath = resolveRoutePath(rootPath, routeMenu.path);
      }
      return retnPath;
    }
    // 默认展开的导航路径
