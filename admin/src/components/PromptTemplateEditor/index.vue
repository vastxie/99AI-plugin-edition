<script setup lang="ts">
  ...
  // 删除选项（仅用于 select）
  const removeOption = (fieldId: string, optionIndex: number | string) => {
    fields.value = fields.value.map((field) => {
      if (field.id === fieldId && field.type === 'select' && field.options) {
        const options = [...field.options];
        if (options.length > 1) {
          // 至少保留一个选项输入框
          options.splice(Number(optionIndex), 1);
          return { ...field, options };
        }
  ...
  // 更新选项值
  const updateOptionValue = (fieldId: string, optionIndex: number | string, value: string) => {
    fields.value = fields.value.map((field) => {
      if (field.id === fieldId && field.type === 'select' && field.options) {
        const options = [...field.options];
        options[Number(optionIndex)] = value;
        return { ...field, options };
      }
      return field;
    });
                    ...
                    <div class="space-y-2 w-full">
                      ...
                      >
                        <el-input
                          :model-value="option"
                          @update:modelValue="
                            (val: string) => updateOptionValue(field.id, index, val)
                          "
                          placeholder="选项内容"
                          size="small"
                          clearable
...
<style scoped>
  ...
  .field-item {
    ...
    /* 移除 Element Plus 卡片的默认外边距 */
    margin: 0 !important;
  }

  /* 移除 el-card 的默认内边距，让内容更紧凑 */
