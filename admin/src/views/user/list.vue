...
<script lang="ts" setup>
  ...
  // 当前用户的积分信息
  const currentUserBalance = reactive({
    model3Count: 0,
    model4Count: 0,
    drawMjCount: 0,
  });
  ...
  function handleSendCrami(row: UserItem) {
    ...
    // 保存当前用户的积分信息
    currentUserBalance.model3Count = row.balanceInfo?.model3Count || 0;
    currentUserBalance.model4Count = row.balanceInfo?.model4Count || 0;
    currentUserBalance.drawMjCount = row.balanceInfo?.drawMjCount || 0;

    // 初始化目标积分为当前积分
    targetBalance.model3Count = currentUserBalance.model3Count;
    targetBalance.model4Count = currentUserBalance.model4Count;
    targetBalance.drawMjCount = currentUserBalance.drawMjCount;

    // 重置调整值为0
    formCrami.model3Count = 0;
    formCrami.model4Count = 0;
    formCrami.drawMjCount = 0;
  }
  ...
  async function handlerResetUserPass(row: any) {
    ...
    const { value: password } = await ElMessageBox.prompt(
      `为用户 ${email || id} 设置新的强密码。保存后其现有会话会全部失效。`,
      '重置密码',
      {
        ...
        inputValidator: (value: string) => value.length >= 12 || '密码至少需要 12 位',
      },
    );
    const res: any = await ApiUser.resetUserPassword({ id, password });
    if (res.success) {
      ElMessage({ type: 'success', message: '密码重置成功，用户现有会话已失效。' });
    }
  }
  ...
  async function handlerSubmitSend(formEl: FormInstance | undefined) {
    formEl?.validate(async (valid) => {
      ...
      // 计算实际的增减值
      const adjustData = {
        model3Count: targetBalance.model3Count - currentUserBalance.model3Count,
        model4Count: targetBalance.model4Count - currentUserBalance.model4Count,
        drawMjCount: targetBalance.drawMjCount - currentUserBalance.drawMjCount,
      };

      await ApiUser.sendUserCrami({ ...adjustData, userId: activeUserId.value });
  ...
  <div>
      ...
      <el-table v-loading="loading" border :data="tableData" style="width: 100%" size="large">
        ...
        <el-table-column fixed="right" label="操作" width="250" align="center">
          <template #default="scope">
            ...
            <el-button link type="danger" @click="handlerResetUserPass(scope.row)">
              重置密码
            </el-button>
    ...
    >
      ...
      <div class="mb-4 p-3 bg-gray-50 rounded">
        <div class="text-sm text-gray-600 mb-2">当前积分</div>
        <div class="grid grid-cols-3 gap-4 text-sm">
          <div>
            <span class="text-gray-500">基础：</span>
            <span class="font-bold">{{ currentUserBalance.model3Count }}</span>
          </div>
          <div>
            <span class="text-gray-500">高级：</span>
            <span class="font-bold">{{ currentUserBalance.model4Count }}</span>
          </div>
          <div>
            <span class="text-gray-500">绘画：</span>
            <span class="font-bold">{{ currentUserBalance.drawMjCount }}</span>
          </div>
        </div>
      </div>

      <!-- 调整后的积分编辑 -->
      <el-form ref="cramiRef" :model="targetBalance" label-width="90px">
        <el-form-item label="基础积分">
          <div class="flex items-center gap-2">
            <el-button @click="targetBalance.model3Count -= 100" size="small">-100</el-button>
            <el-button @click="targetBalance.model3Count -= 10" size="small">-10</el-button>
            <el-input-number
              v-model="targetBalance.model3Count"
              :min="0"
              :max="99999999"
              :step="1"
              step-strictly
        ...
        <el-form-item label="高级积分">
          <div class="flex items-center gap-2">
            <el-button @click="targetBalance.model4Count -= 100" size="small">-100</el-button>
            <el-button @click="targetBalance.model4Count -= 10" size="small">-10</el-button>
            <el-input-number
              v-model="targetBalance.model4Count"
              :min="0"
              :max="99999999"
              :step="1"
              step-strictly
        ...
        <el-form-item label="绘画积分">
          <div class="flex items-center gap-2">
            <el-button @click="targetBalance.drawMjCount -= 100" size="small">-100</el-button>
            <el-button @click="targetBalance.drawMjCount -= 10" size="small">-10</el-button>
            <el-input-number
              v-model="targetBalance.drawMjCount"
              :min="0"
              :max="99999999"
              :step="1"
              step-strictly
