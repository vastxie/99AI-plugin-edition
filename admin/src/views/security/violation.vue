      ...
      >
        ...
        <el-table-column prop="status" label="用户状态" width="120" align="center">
          <template #default="{ row }: { row: any }">
            <el-tag
              :type="
                row.userInfo.isVisitor
                  ? 'warning'
                  : USER_STATUS_TYPE_MAP[row.userInfo.status as UserStatus]
              "
              v-if="row.userInfo"
            >
              <!-- 优先使用后端返回的 statusText，特别是针对游客 -->
              {{
                row.userInfo.isVisitor
                  ? row.userInfo.statusText || '游客'
        ...
        <el-table-column fixed="right" label="操作" width="120" align="center">
          <template #default="scope">
            <el-button
              ...
              v-if="scope.row.userInfo && !scope.row.userInfo.isVisitor"
            >
              变更用户状态
            </el-button>
            <el-tag
              v-else-if="scope.row.userInfo && scope.row.userInfo.isVisitor"
              type="info"
              size="small"
            >
              游客用户
