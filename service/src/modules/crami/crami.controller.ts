...
export class CramiController {
  ...
  @Get('queryCramiUseDetail')
  @ApiOperation({ summary: '查询卡密使用详情' })
  @UseGuards(SuperAuthGuard)
  @ApiBearerAuth()
  async queryCramiUseDetail(@Query('id') id: number, @Req() req: Request) {
    return this.cramiService.queryCramiUseDetail(id, req);
  }
}
