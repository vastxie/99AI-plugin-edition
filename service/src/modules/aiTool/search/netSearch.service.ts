...
export class NetSearchService {
  ...
  async processNetSearch(
    ...
    try {
      Logger.log(`[网络搜索] 开始搜索，关键词长度: ${prompt?.length || 0}`, 'NetSearchService');
  ...
  async webSearchPro(prompt: string) {
    try {
      ...
      if (!pluginUrl || !pluginKey) {
        Logger.warn('搜索插件配置缺失', 'NetSearchService');
        return { searchResults: [], images: [] };
      }
      ...
      if (!response.ok) {
        Logger.error(`[搜索] 接口返回错误: ${response.status}`, 'NetSearchService');
        return { searchResults: [], images: [] };
      }

      const apiResult = await response.json();
      // 移除详细日志，只在调试时使用
      Logger.debug(`[搜索] 收到响应，处理中...`, 'NetSearchService');
      ...
      Logger.log(`[搜索] 完成，返回 ${formattedResult.length} 条结果`, 'NetSearchService');
      ...
      Logger.error(
        `[搜索] 调用接口出错: ${handleError(fetchError)}`,
        fetchError?.stack,
        'NetSearchService',
      );
      return {
