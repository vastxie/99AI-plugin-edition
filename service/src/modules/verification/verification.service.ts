...
export class VerificationService {
  constructor(
    ...
    private readonly verificationEntity: Repository<VerificationEntity>,
  ...
  async verifyCode(
    ...
    const v: VerificationEntity = await this.verificationEntity.findOne({
      where: { id, type },
      order: { createdAt: 'DESC' },
    });
    if (!v) {
    ...
    const result = await this.verificationEntity.update(
      { id, used: VerificationUseStatusEnum.UNUSED },
